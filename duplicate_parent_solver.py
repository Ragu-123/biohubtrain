...
class DuplicateParentTrackingSolver:
    ...
    def solve_full_volume(
        ...
        for step in range(T - 1):
            ...
            # 1. Apply Kinematic Momentum (Acceleration Residue)
            predicted_coords = src_c.copy()
            for i, nid in enumerate(src_ids):
                if nid in active_velocities:
                    v_prev = active_velocities[nid]
                    
                    # Density-Adaptive Lambda for 0.97+ score
                    ...
                    if hasattr(self, 'ADAPTIVE_LAMBDA') and self.ADAPTIVE_LAMBDA:
                        # Estimate local density using src_c neighbors
            ...
            # Update Velocity Map for the NEXT frame
            for e in solved_pair:
                p_id = src_ids[e.source_idx]
                c_id = tgt_ids[e.target_idx]
                v_curr = (tgt_c[e.target_idx] - src_c[e.source_idx]) * np.array(self.voxel_scale)
                active_velocities[c_id] = v_curr
