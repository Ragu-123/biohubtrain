...
def track_volume_inference(
        ...
        for f_idx in range(window_size - 1):
            ...
            for eff_score, raw_prob, i, j, v_curr, speed_curr in candidates:
                ...
                if n_ch == 0:
                    ...
                    if (
                        dist_um > 8.54 or dist_sisters > 15.34 or symmetry_ratio > 0.85
                        or cos_spindle > -0.25 or midpoint_offset > 4.50
                    ):
                        continue
                    gi, gj = int(idx_src[i]), int(idx_tgt[j])
                    all_edges.append((gi, gj, float(raw_prob), dist_um))
    ...
    edges_clean = prune_false_divisions(coords_clean, edges_clean, scale=scale)
